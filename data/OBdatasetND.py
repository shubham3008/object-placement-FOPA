...
class CPDatasetND(Dataset):
    def __init__(self, file, bg_dir, fg_dir, mask_dir, in_size, datatype='train'):
        ...
        self.train_triple_transform = ComposeND([JointResizeND(in_size)])
    ...
    def __getitem__(self, index):
        ...
        bg_nrm = Image.open(bg_path.replace('/bg','/bg_normals'))
        bg_dpt = Image.open(bg_path.replace('/bg','/bg_depth'))
...
#         bg_nrm_t = self.train_img_transform(bg_nrm_t)
        bg_nrm_t = self.train_mask_transform(bg_nrm_t)
        bg_dpt_t = self.train_mask_transform(bg_dpt_t)

        if is_flip == True:
            fg_t = self.transforms_flip(fg_t)
            bg_t = self.transforms_flip(bg_t)
            mask_t = self.transforms_flip(mask_t)
            bg_nrm_t = self.transforms_flip(bg_nrm_t)
            bg_dpt_t = self.transforms_flip(bg_dpt_t)
        ...
        return i, bg_t, bg_nrm_t, bg_dpt_t, mask_t, fg_t, target_t.squeeze(), labels_num, composite_list, feature_pos, w, h, save_name
...
def create_loader(table_path, bg_dir, fg_dir, mask_dir, in_size, datatype, batch_size, num_workers, shuffle):
    dset = CPDatasetND(table_path, bg_dir, fg_dir, mask_dir, in_size, datatype)
