...
from data.OBdatasetND import create_loader
...
class Trainer:
    def __init__(self, args):
        ...
        shutil.copy(f"{proj_root}/trainND.py", self.path["trainer_log"])
        shutil.copy(f"{proj_root}/data/OBdatasetND.py", self.path["dataset_log"])
        shutil.copy(f"{proj_root}/network/ObPlaNetND.py", self.path["network_log"])
        ...
        self.net = nn.DataParallel(ObPlaNet_resnet18(pretrained=True)).to(self.dev)
    ...
    def train(self):
        ...
        for curr_epoch in range(self.start_epoch, self.end_epoch):
            ...
            for train_batch_id, train_data in enumerate(self.tr_loader):
                ...
                _, train_bgs, train_bg_nrms, train_bg_dpts, train_masks, train_fgs, train_targets, num, composite_list, feature_pos, _, _, _ = train_data
         
                train_bgs = train_bgs.to(self.dev, non_blocking=True)
                train_bg_nrms = train_bg_nrms.to(self.dev, non_blocking=True)
                train_bg_dpts = train_bg_dpts.to(self.dev, non_blocking=True)
                ...
                # model training
                train_outs, feature_map = self.net(train_bgs, train_bg_nrms, train_bg_dpts, train_fgs, train_masks, 'train')
