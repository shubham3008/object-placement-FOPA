...
from data.OBdatasetND import create_loader, _collect_info
from data.OBdatasetND import make_composite_PIL

'''
F-1 Measure: 0.732885, 
Weighted acc measure: 0.798437, 
'''
    ...
    def evalutate_model(self, datatype):
        ...
        with torch.no_grad():
            ...
            for _, test_data in enumerate(tqdm(self.ts_loader)):
                _, test_bgs, test_bg_nrms, test_bg_dpts, test_masks, test_fgs, test_targets, nums, composite_list, feature_pos, _, _, _ = test_data
                test_bgs = test_bgs.to(self.dev, non_blocking=True)
                test_bg_nrms = test_bg_nrms.to(self.dev, non_blocking=True)
                test_bg_dpts = test_bg_dpts.to(self.dev, non_blocking=True)
                ...
                test_outs, _ = self.net(test_bgs, test_bg_nrms, test_bg_dpts, test_fgs, test_masks, 'val')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--path', type=str, default= "demo2024-10-10-17:20:47.369235")
